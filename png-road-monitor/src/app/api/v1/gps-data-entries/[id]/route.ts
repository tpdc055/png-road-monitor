...
const USE_MOCK_DATA = process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true' || process.env.NODE_ENV === 'development' || !prisma;
  ...
  try {
    ...
    const updatedEntry = await prisma.gpsDataEntry.update({
      ...
      data: {
        ...
        taskId: taskId || null,
        latitude: latitude ? Number.parseFloat(latitude.toString()) : existingEntry.latitude,
        longitude: longitude ? Number.parseFloat(longitude.toString()) : existingEntry.longitude,
        chainage: chainage !== undefined ? Number.parseFloat(chainage.toString()) : existingEntry.chainage,
        workType: workType || existingEntry.workType,
        status: status ? status.toUpperCase().replace(/\s+/g, '_') : existingEntry.status,
    ...
    const transformedEntry = {
      ...
      status: updatedEntry.status.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
