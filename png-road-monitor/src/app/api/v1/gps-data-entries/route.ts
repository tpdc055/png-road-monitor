  ...
  try {
    ...
    if (USE_MOCK_DATA) {
      ...
      const mockEntries = Array.from({ length: 25 }, (_, i) => ({
        ...
        phase: ['Preliminary/Survey', 'Earthworks', 'Subbase & Base Construction', 'Pavement & Sealing'][Math.floor(Math.random() * 4)],
        task: 'Survey & Pegging',
        chainage: Math.random() * 50,
        latitude: -6.314993 + (Math.random() - 0.5) * 0.01,
        longitude: 143.95555 + (Math.random() - 0.5) * 0.01,
        status: ['Completed', 'In Progress', 'Inspection Required', 'Approved'][Math.floor(Math.random() * 4)],
      ...
      if (status && status !== 'all') {
        filteredEntries = filteredEntries.filter(entry => entry.status.toLowerCase() === status.toLowerCase());
      }
