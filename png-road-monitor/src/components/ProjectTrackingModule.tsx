...
export default function ProjectTrackingModule({ selectedProjectId, userRole = 'SITE_ENGINEER' }: ProjectTrackingProps) {
  ...
  const calculateCompletionPercentage = (project: Project) => {
    if (!project.totalDistance || project.totalDistance === 0) return project.progress;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Project Tracking Module</h1>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span>Progress</span>
                    <span className={`font-medium ${getProgressColor(calculateCompletionPercentage(project))}`}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full transition-all duration-300 ${
                        calculateCompletionPercentage(project) >= 70 ? 'bg-green-500' :
                        calculateCompletionPercentage(project) >= 40 ? 'bg-blue-500' : 'bg-red-500'
                      }`}
                      style={{ width: `${Math.min(calculateCompletionPercentage(project), 100)}%` }}
                    />
                  </div>
                </div>
                ...
                {project.totalDistance && (
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium">
                      {(project.completedDistance || 0).toFixed(1)}km / {project.totalDistance.toFixed(1)}km
                    </span>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-gray-600">
                    {(selectedProject.completedDistance || 0).toFixed(1)}km of {(selectedProject.totalDistance || 0).toFixed(1)}km
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-lg sm:text-2xl font-bold text-green-600">
                    {formatCurrency(selectedProject.contractValue || selectedProject.budget)}
                  </div>
                  <div className="text-xs text-gray-600">
                    Spent: {formatCurrency(selectedProject.spent)} ({((selectedProject.spent / selectedProject.budget) * 100).toFixed(1)}%)
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xl sm:text-2xl font-bold text-purple-600">
                    {selectedProject.startDate ?
                      Math.ceil((new Date(selectedProject.endDate!).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
                      : 'TBD'
                    }
                  </div>
                  <div className="text-xs text-gray-600">
