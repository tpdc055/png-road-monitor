...
const FEEDBACK_CATEGORIES = {
  'CONSTRUCTION_QUALITY': {
    ...
    description: 'Issues with workmanship, materials, or construction standards',
    icon: Building,
...
const FEEDBACK_TYPES = {
  'COMPLAINT': { label: 'Complaint', color: 'bg-red-100 text-red-800', icon: AlertTriangle },
  'SUGGESTION': { label: 'Suggestion', color: 'bg-blue-100 text-blue-800', icon: MessageSquare },
  'COMPLIMENT': { label: 'Compliment', color: 'bg-green-100 text-green-800', icon: ThumbsUp },
  'CONCERN': { label: 'Concern', color: 'bg-yellow-100 text-yellow-800', icon: Flag },
  'REQUEST': { label: 'Request', color: 'bg-purple-100 text-purple-800', icon: Plus }
};
...
const STATUS_OPTIONS = {
  ...
  'INVESTIGATING': { label: 'Investigating', color: 'bg-orange-100 text-orange-800', icon: Search },
  'RESOLVED': { label: 'Resolved', color: 'bg-green-100 text-green-800', icon: CheckCircle },
  'CLOSED': { label: 'Closed', color: 'bg-gray-100 text-gray-800', icon: FileText }
};

export default function CommunityFeedbackSystem({ userRole = 'COMMUNITY_LIAISON', userId, projectId }: CommunityFeedbackSystemProps) {
      ...
      const mockFeedbacks: CommunityFeedback[] = [
        {
          ...
          description: 'The road surface is developing cracks after only 2 months of completion. Water is pooling in these areas during rain.',
          priority: 'HIGH',
          status: 'UNDER_REVIEW',
        ...
        },
        {
          ...
          category: 'SAFETY',
          description: 'We need better lighting along the road near the school area. Children walk this route early in the morning and evening.',
          priority: 'MEDIUM',
        ...
        },
        {
          ...
          description: 'Construction activities have affected the water source in our village. The water is now muddy and not safe to drink.',
          priority: 'URGENT',
          status: 'SUBMITTED',
        ...
        },
        {
          ...
          description: 'The new road has greatly improved our access to the market. We can now transport our produce easily and get better prices.',
          priority: 'LOW',
          status: 'RESOLVED',
          resolution: 'Feedback acknowledged and shared with project team. Positive impact documented for project reporting.',
  ...
  const handleSubmitFeedback = async () => {
    try {
      const newFeedback: CommunityFeedback = {
        ...
        submittedBy: feedbackForm.isAnonymous ? 'Anonymous' : feedbackForm.submittedBy,
  ...
  const getFeedbackStats = () => {
    const stats = {
      ...
      byStatus: {
        ...
        underReview: feedbacks.filter(f => f.status === 'UNDER_REVIEW').length,
        investigating: feedbacks.filter(f => f.status === 'INVESTIGATING').length,
      ...
      byType: {
        ...
        compliments: feedbacks.filter(f => f.feedbackType === 'COMPLIMENT').length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Community Feedback System</h1>
          <p className="text-gray-600 mt-1">
            Collect, manage, and respond to community feedback on Connect PNG projects
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-orange-600">{stats.byStatus.submitted + stats.byStatus.underReview}</div>
        ...
        <TabsContent value="dashboard" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            {Object.entries(FEEDBACK_TYPES).map(([key, type]) => {
              const count = stats.byType[key.toLowerCase() as keyof typeof stats.byType] || 0;
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest community feedback submissions</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {feedbacks.slice(0, 5).map((feedback) => {
                  ...
                  return (
                    <div key={feedback.id} className="flex items-start gap-4 p-4 border rounded-lg hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        {feedback.resolution && (
                          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                            ...
                            <div className="text-green-700">{feedback.resolution}</div>
                            {feedback.resolutionDate && (
                              <div className="text-sm text-green-600 mt-1">
                                Resolved on {feedback.resolutionDate.toLocaleDateString()}
                              </div>
                            )}
                          </div>
                        )}
        ...
        <TabsContent value="submit" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Share your feedback, concerns, or suggestions about Connect PNG projects
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="submitter-name">Your Name {!feedbackForm.isAnonymous && '*'}</Label>
                  <Input
                    id="submitter-name"
                    value={feedbackForm.submittedBy}
                    onChange={(e) => setFeedbackForm(prev => ({ ...prev, submittedBy: e.target.value }))}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFeedbackForm(prev => ({ ...prev, submitterType: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFeedbackForm(prev => ({ ...prev, feedbackType: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFeedbackForm(prev => ({ ...prev, priority: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFeedbackForm(prev => ({ ...prev, contactPreference: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex gap-2 pt-4">
                <Button
                  ...
                  disabled={!feedbackForm.description || (!feedbackForm.isAnonymous && !feedbackForm.submittedBy)}
                >
